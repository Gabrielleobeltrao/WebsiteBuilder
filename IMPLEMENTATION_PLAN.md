...
2026-08-12 19:02 | Promotion gate | A pre-promotion E2E run failed a visual-regression spec. It was not flakiness: allocateSlug reads then writes, and the unique index settles the tie, so two projects created with the same name in the same instant left one caller with SlugTakenError instead of a site — for a collision the product already resolves for anyone not unlucky with timing. create() now retries the allocate-and-insert on a duplicate key, bounded at five. project-repository.test.ts creates eight "Portfolio" projects concurrently and asserts eight distinct slugs; it fails with the exact prior error when the fix is stashed. The E2E fixture names were also colliding across workers (a per-process counter plus Date.now()) and now use a random suffix | npm run typecheck 0; npm run test 0 with 2,068 tests; npm run build 0; npm run test:e2e 0 with 95 passed | —
```
