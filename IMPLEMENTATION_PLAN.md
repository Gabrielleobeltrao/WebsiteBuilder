...
- [x] **P9-T7 — Responsive image art direction**
...
- [x] **P18-T2 — Deterministic publication builder and preflight**
...
- [x] **P18-T7 — Environment validation and Coolify production manifests**
...
- [x] **P18-T8 — Operational hardening, backups, and publication smoke tests**
...
| 2026-08-10 | P18-T2 | The published content hash covers only what a visitor observes, not the stored document | Hashing revision, timestamps and the feature projection made every republish of an unchanged site look like new content, which defeats the only question the hash exists to answer. The repository keeps a separate integrity check that the stored snapshot round-trips. |
| 2026-08-10 | P18-T2 | A router declares the weakest permission it needs and each writing route demands its own | Mounting a router with one permission made `media:read` sufficient to upload and delete media, and `project:read` sufficient to create, rename and delete projects. Found while wiring publishing. |
...
| 2026-08-11 | Deployment | Image build stages set `NODE_ENV=production`, and devDependencies are requested with `--include=dev` | Setting `development` to answer a platform warning about npm shipped a development bundle to production: Vite reads NODE_ENV for its build mode, so `import.meta.env.DEV` was true and every guard reading it was inverted, including the one that permits `http://` links. npm 10 does not omit devDependencies for NODE_ENV at all, so the warning being answered did not apply. A test now reads the built bundle and fails on a development build. |
## 15. Progress Log
