...
async function openBuilder(page: Page): Promise<void> {
  ...
  await page.getByLabel("Email").fill(`e2e-visual-${Date.now()}-${counter}@example.test`);
  await page.getByLabel("Password", { exact: true }).fill("a-long-enough-password");
  await page.getByRole("button", { name: /Create|Sign up|Continue/i }).click();
  await expect(page).toHaveURL(/\/app\//, { timeout: 20_000 });

  // Unique across the whole run: a site's slug becomes its subdomain, which is claimed platform-wide,
  // so two accounts creating the same name is a refusal rather than two sites. The name is masked in
  // the screenshot, which is what lets it vary.
  const site = `Screenshot Site ${Date.now()}-${counter}`;
