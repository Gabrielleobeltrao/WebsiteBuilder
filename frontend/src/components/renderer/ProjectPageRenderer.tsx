...
export function SectionRenderer({ section, breakpointId }: { section: BuilderSection; breakpointId?: string }) {
  if (section.hidden) return null;

  return (
    <section style={sectionStyle(section, breakpointId)} data-section-id={section.id}>
 ...
 */
export function ProjectPageRenderer({ page, breakpointId }: { page: BuilderPage; breakpointId?: string }) {
  return (
    <div style={{ backgroundColor: page.canvas.backgroundColor, minHeight: page.canvas.minHeight }}>
      {page.sections.map((section) => (
        <SectionRenderer key={section.id} section={section} breakpointId={breakpointId} />
      ))}
    </div>
  );
}
