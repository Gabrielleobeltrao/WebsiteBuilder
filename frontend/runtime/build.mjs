/**
 * Builds the published-site interaction runtime and writes it into the backend as a source constant.
 *
...
const output = join(here, "..", "..", "backend", "src", "renderer", "runtime.generated.ts");

/** Builds the runtime and returns its minified source. */
export async function buildRuntime() {
  ...
  if (file === undefined) throw new Error("the runtime build produced no output");
  return file.text;
}

export function runtimeModule(source) {
  const hash = createHash("sha256").update(source).digest("hex").slice(0, 16);

  return `/**
 * The published-site analytics runtime, built from \`frontend/runtime/src/index.ts\`.
 *
 * Generated — do not edit. Run \`npm run build:runtime\` after changing the source; a test fails if
...
// Only when run as a command. The drift test imports `buildRuntime` to compare against the
...
if (process.argv[1] !== undefined && fileURLToPath(import.meta.url) === process.argv[1]) {
  ...
  await writeFile(output, runtimeModule(source), "utf8");
  process.stdout.write(`runtime built: ${Buffer.byteLength(source, "utf8")} bytes minified\n`);
}
